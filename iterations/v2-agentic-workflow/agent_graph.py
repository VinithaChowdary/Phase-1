...
def log_node(node_name: str, message: str):
    timestamp = datetime.now().strftime("%H:%M:%S")
    print(f"{Fore.MAGENTA}[{timestamp}] 📍 [{node_name}] {message}{Style.RESET_ALL}")
...
def log_state(message: str):
    timestamp = datetime.now().strftime("%H:%M:%S")
    print(f"{Fore.WHITE}{Back.BLUE}[{timestamp}] 📊 [STATE] {message}{Style.RESET_ALL}")

# Load environment variables
load_dotenv()

log_info("🚀 Initializing Agentic Workflow System...")
...
# Build workflow
log_info("🔨 Building LangGraph workflow...")
builder = StateGraph(AgentState)

# Add nodes
log_info("Adding nodes to graph...")
...
log_info("Setting up memory persistence...")
memory = MemorySaver()
