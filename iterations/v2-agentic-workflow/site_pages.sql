...
-- Create the documentation chunks table (if not exists)
create table if not exists site_pages (
    ...
    embedding vector(384),  -- Local embeddings are 384 dimensions
...
drop function if exists match_site_pages(vector(384), int, jsonb);
...
alter table site_pages enable row level security;
 ...
 drop policy if exists "Allow data ingest" on site_pages;
create policy "Allow data ingest"
  on site_pages
  for insert
  to public
  with check (true);
