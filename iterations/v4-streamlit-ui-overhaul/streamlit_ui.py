...
# Set custom theme colors to match meta_agent logo (green and pink)
...
def generate_mcp_config(ide_type):
    ...
    if ide_type == "Windsurf":
        ...
        return f"{python_path} {server_script_path}", f"docker run --rm -p 8100:8100 meta_agent:latest python mcp_server.py"
    elif ide_type == "Cline":
...
def mcp_tab():
    ...
    if st.session_state.selected_ide:
        ...
        with config_tab2:
            ...
            st.markdown("- Must be running meta_agent not within a container")           
...
async def chat_tab():
    """Display the chat interface for talking to meta_agent"""
...
def intro_tab():
    """Display the introduction and setup guide for meta_agent"""
    # Display the banner image
    st.image("public/meta_agent.png", use_container_width=True)
    
    # Welcome message
    st.markdown("""
    # Welcome to meta_agent!
    
    meta_agent is an AI meta-agent designed to autonomously build, refine, and optimize other AI agents.
    
    It serves both as a practical tool for developers and as an educational framework demonstrating the evolution of agentic systems.
    meta_agent is developed in iterations, starting with a simple Pydantic AI agent that can build other Pydantic AI agents,
    all the way to a full agentic workflow using LangGraph that can build other AI agents with any framework.
    
    Through its iterative development, meta_agent showcases the power of planning, feedback loops, and domain-specific knowledge in creating robust AI agents.
    """)
    
    # Setup guide with expandable sections
    st.markdown("## Setup Guide")
    st.markdown("Follow these concise steps to get meta_agent up and running (IMPORTANT: come back here after each step):")
    ...
    with st.expander("Step 1: Environment Configuration", expanded=True):
        ...
        These settings determine how meta_agent connects to external services and which models it uses.
        """)
    ...
    with st.expander("Step 2: Database Setup", expanded=False):
        ...
        meta_agent uses Supabase for vector storage and retrieval:
    ...
    with st.expander("Step 5: MCP Configuration (Optional)", expanded=False):
        ...
        This enables you to use meta_agent directly from your AI-powered IDE.
        """)
    ...
    # Step 6: Using meta_agent
    with st.expander("Step 6: Using meta_agent", expanded=False):
        st.markdown("""
        ### Using meta_agent
        ...
        3. meta_agent will plan and generate the necessary code
        
        You can also use meta_agent directly from your AI IDE if you've configured MCP.
        """)
    ...
    - [GitHub Repository](https://github.com/coleam00/meta_agent)
    - [meta_agent Community Forum](https://thinktank.ottomator.ai/c/meta_agent/30)
            ...
            with col1:
                ...
                if st.button("Crawl Pydantic AI Docs", key="crawl_pydantic") and not (st.session_state.crawl_tracker and st.session_state.crawl_tracker.is_running):
                    try:
                        ...
                        from meta_agent.crawl_pydantic_ai_docs import start_crawl_with_requests
                ...
                if st.button("Clear Pydantic AI Docs", key="clear_pydantic"):
                    with st.spinner("Clearing existing Pydantic AI docs..."):
                        try:
                            ...
                            from meta_agent.crawl_pydantic_ai_docs import clear_existing_records
...
def database_tab():
    ...
    st.write("Set up and manage your Supabase database tables for meta_agent.")
...
def agent_service_tab():
    ...
    st.write("Start, restart, and monitor the meta_agent agent service for MCP.")
    ...
    with st.container():
        ...
        st.text_area(
            label="Realtime Logs from meta_agent Service",
            value=output_text,
...
def environment_tab():
    ...
    st.write("- Configure your environment variables for meta_agent. These settings will be saved and used for future sessions.")
