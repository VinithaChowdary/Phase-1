# Meta_agent V6 - Tool Library and MCP Integration

This is the sixth iteration of the Meta_agent project, building upon V5 by implementing a comprehensive library of prebuilt tools, examples, and MCP server integrations. The system retains the multi-agent coding workflow with specialized refiner agents from V5, but now adds a powerful advisor agent that can recommend and incorporate prebuilt components.

What makes V6 special is its approach to reducing development time and hallucinations through component reuse. The advisor agent can now analyze the user's requirements and recommend relevant prebuilt tools, examples, and MCP server integrations from the agent-resources library. This significantly enhances Meta_agent's capabilities and reduces the need to create everything from scratch.
...
9. **Finalization**: Meta_agent provides the complete code with execution instructions

### Agent Graph

The LangGraph workflow orchestrates the entire process:

![Meta_agent Graph](../../public/Meta_agentGraph.png)
...
1. Start a conversation with Meta_agent and describe the agent you want to create
2. Meta_agent will automatically analyze your requirements through the advisor agent
...
6. When satisfied, ask Meta_agent to finalize the agent

## Core Files

### Advisor Components
- `meta_agent/advisor_agent.py`: Agent that recommends relevant prebuilt components
- `meta_agent/agent-resources/`: Directory containing prebuilt tools, examples, and MCP configurations

### Refiner Agents
- `meta_agent/refiner_agents/tools_refiner_agent.py`: Enhanced to validate MCP configurations

### Workflow Orchestration
- `meta_agent/meta_agent_graph.py`: Updated LangGraph workflow with advisor integration
...
```bash
git clone https://github.com/coleam00/meta_agent.git
cd meta_agent
```

2. Run the Docker setup script:
```bash
# This will build both containers and start Meta_agent
