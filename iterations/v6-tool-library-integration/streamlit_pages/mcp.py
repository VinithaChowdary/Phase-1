...
def generate_mcp_config(ide_type, python_path, server_script_path):
    ...
    if ide_type == "Windsurf":
        ...
        return f"{python_path} {server_script_path}", f"docker run -i --rm -e GRAPH_SERVICE_URL=http://host.docker.internal:8100 meta_agent-mcp:latest"
...
def mcp_tab():
    ...
    if st.session_state.selected_ide:
        ...
        if selected_ide == "Windsurf":
            ...
            1. Deploy and run Meta_agent in Docker
            2. In the Meta_agent UI, start the MCP service.
            3. Open a terminal and navigate to your work folder.
            4. Execute the command: 
            
            \tFor Docker: `claude mcp add Meta_agent docker run -i --rm -e GRAPH_SERVICE_URL=http://host.docker.internal:8100 meta_agent-mcp:latest `   
            \tFor Python: `claude mcp add Meta_agent {python_path} {server_script_path}`
            
            5. Start Claude Code with the command `claude`. When Claude Code starts, at the bottom of the welcome section will be a listing of connected MCP Services, Meta_agent should be listed with a status of _connected_.
            6. You can now use the Meta_agent MCP service in your Claude Code projects
